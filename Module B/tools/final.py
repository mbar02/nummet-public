...
# steps = np.array([1, 2, 4]) # Wolff and Metropolis
steps = np.array([20, 50, 100]) # Multicluster
...
def config():
  setting = {
    ...
    'UPS_SIM'        : lambda alg, n, beta, g: int( 20 if alg == 'multicluster' else 2 * np.sqrt(n) ),
    'SAMPLES_SIM'    : lambda alg, n, beta, g:
      int(np.ceil(
        ( np.exp(tau(g, maxn)*(energy(g, maxn)-energy(g, (maxn+1)%2+1))) / eta_sys ) ** 2
      * n / 3000 ) ) * 10, # it was /300 before
    ...
    'OUTPUT_DIR'     : '/home/contazzi/local/final-aho-high-n',
