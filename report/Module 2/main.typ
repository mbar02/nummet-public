...
whereas our estimator for it's mean value $angle.l O angle.r$, given $N_s$ samples will be:
$ hat(O) = 1/N_s sum_(n = 1) ^(N_s) #h(0.5em) overline(O)_n $
The following subsections describe in detail how we computed statistical uncertainties and how we used the GEVP @gevp to find the first four energy gaps, using the first four powers of $x$ as observables.
...
+ starting from raw samples of primary observables, we block data to make them independent. Note that the number of samples reduces to $N_s^"eff"$;
+ we compute sample covariance between the primary observables;
+ we use the first order error propagation formula:
  $ sigma^2 _f = sum (diff f)/(diff x_i) dot (diff f)/(diff x_j) dot "cov"(x_i,x_j) . $

== Finding energy gaps
With regard to @def_correlator, in the low temperature limit ($beta -> +oo$) the correlator has the form:
$ C_(i j)^oo (t) = sum_(n=0)^(+oo) braket(0,O_i ,n) braket(n, O_j , 0) e^(-t(E_n - E_0)) $
Then in the large $t$ limit (see below for clarification), we neglect contribution over the fourth gap, hence the solution to @gevp is _exactly_:
$ lambda_n = exp[-tau(E_n - E_0)] , $
which can be inverted to find gaps.
This approximations introduce the following systematic errors:
- finite $beta$ introduces an error on each entry of the correlators' matrix:
  $ C_(i,j) (t) approx C_(i j)^oo (1 + O(e^(-beta (E_1 - E_0)))) $
- finite $t$ introduces an error on the eigenvalues:
  $lambda$

== Systematic errors on the gaps
<sys_gaps>
