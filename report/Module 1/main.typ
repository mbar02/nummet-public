...
The only observables that we need for our purposes are the average magnetization $m$.
...
=== Wolff's MC
a single-cluster algorithm that is hard to parallelize but much more efficient than Metropolis, especially close to the critical point. This program was written in plain `C`.
...
== Lattices
We carried out simulations on two-dimensional lattices with periodic, untwisted boundary conditions.
...
```)
containing the spin value and neighbors. We noticed an important performance increase using pointers to neighbors instead of indices with the Wolff algorithm#footnote([By using indices instead of pointers, the run time worsens by a factor of 1.5.]), but the opposite in the case of Metropolis.
...
$ ($m$ does not depend on $i$ because of invariance by translations).
...
Therefore we estimate the values of critical temperature and exponents by the following procedure:
+ chosen the lattice side $L$, sample the statistical distribution at various $beta$;
+ for each sample, we compute the magnetic susceptibility, and we identify its peak coordinates via a quadratic fit;
...
We take into account the following systematic errors:
...
- To find the maximum of $chi_L$, we sample it around its maximum and perform a fit with a parabolic model. 
  
  In order to neglect the latter, we tune simulation parameters so that the statistical error is about 10 times larger than the systematic one (and so the uncertainties could be estimated via a standard statistical data analysis), proceeding as follows:
...
Secondly, it is desirable to save as little data as possible, as it takes up time and memory storage to do so.
...
In order to characterize it, after finding the critical temperature as explained in @critical_temperature_and_exponents, we perform simulations at $beta_"cr"$ for different $L$s, as showed in [INSERIRE GRAFICO DELLE AUTOCORRELAZIONI].
Then, we extract $tau_"exp"$ via an exponential fit; finally, we estimate $z$ by means of another exponential fit, as shown in [INSERIRE GRAFICO FIT DI Z]
...
It can be shown that the variance of this random variables~is:
...
As we increase $k$, the measures in the reduced sample get more and more uncorrelated, until the sample variance of the mean saturates to $sigma^2_(overline(O))$, as shown in [INSERIRE GRAFICO DI BLOCKING].

Finally, we can compute the sample variance $sigma^2_F$ and use @tau_int to find $tau_"int"^O$.

== Statistical uncertainties of secondary observables
For the purposes of this report, we only wanted reliable estimates of the uncertainties for the magnetic susceptibility observable,
as it serves to fit the critical inverse temperature $beta_"cr"$ and the critical exponents $gamma$ and $nu$.
Being it a secondary observables (it depends on both $m^2$ and $|m|$), we use a Jackknife method: using the blocked data, we start from $N$ measures
...
Thanks to the attention paid in the choice of the sampling intervals of $beta$  and , most of the quadratic fits
...
Nevertheless, as far as efficiency is concerned, Metropolis shows a much heavier critical slowing down close to critical point, and in general much higher correlation times, even though the steps are whole lattice updates
...
TODO: ricontrollare i tempi verbali
Capire perché parte dalla figura 2
mettere i graficini che stanno sul drive
...
#place(scope: "parent", float: true, auto, [
  #figure(
    image("final-plots/chiMax_metropolis_hex_80.svg", width: 50%),
...
#place(scope: "parent", float: true, auto, [
  #figure(
    ...
    caption: [Histograms of the sampled probability of the average magnetization. The four picture are related to a triangular lattice. The two at the top have been sampled with $L=30$, the two at the bottom with $L=300$. At left, $beta=0.261$ (above the critical temperature) and the right ones at $beta=0.282$.]
  )
  <ssb>
])
#place(scope: "parent", float: true, auto, [
  #figure(tableFromCSV("example.csv", cell-inset: (x: 0.8cm), caption: [Experimental results.]
  ))
  <tabellariassuntiva>
])