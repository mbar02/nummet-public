...
We use AoS memory management. This proves to be better not only for code readability, but even more efficient as it allows us to identify neighboring sites via pointers#footnote([Attention must be paid to some subtleties regarding how these addresses are created. For example, code running on the GPU sees different virtual addresses than on the CPU.]).
...
+ chosen the lattice side $L$, sample the statistical distribution at various $beta$s;
...
An interesting conclusion we can extract from this work is the comparison between the Metropolis and Wolff algorithms; both of them are able to simulate the Ising model, giving results that are well in agreement with the theory.
